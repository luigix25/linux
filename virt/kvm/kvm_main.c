...
bool kvm_make_all_cpus_request_vmpl(struct kvm *kvm, unsigned int req, int vmpl)
{
	...
	kvm_for_each_vcpu(i, vcpu, kvm) {
		if (vmpl >= 0) {
			...
			max_vmpl = kvm_x86_call(max_vmpl)(kvm);
			for (request_vmpl = 0; request_vmpl <= max_vmpl; ++request_vmpl) {
				vcpu = vcpu->vcpu_parent->vcpu_vmpl[request_vmpl];
				kvm_make_vcpu_request(vcpu, req, cpus, me);
			}
		}
	}
			  ...
			  struct task_struct *next)
{
	...
	if (task_is_runnable(current) && vcpu->common->wants_to_run) {
